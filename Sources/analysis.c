...
int A[128]={0};
int B[128]={0};
int al_end=0,ar_end=0,bl_end=0,br_end=0;            //左右CCD向左右扫描的终点
int a_start=67,b_start=67;                          //左右CCD扫描起始点
...
int a_allwhite=42,a_allblack=2,b_allwhite=42,b_allblack=2;                  //全白,全黑判断标准
...
void PixelScan(void)
{
	...
	for(i=b_start,j=a_start;i<br_end,j>al_end;i++,j--) //_start待确定 ？？？？？？？？？
	{
		...
		if(A[j]>a_T) //左灯向左扫描跳变沿
					al_count++;
				if(A[j]-A[j-a_scan]>a_value&&A[j-1]-A[j-a_scan-1]>a_value)
				{
					al_flag=2; //白-黑
					al_edge=j-a_halfscan;
					break;
				}
				if(A[j-a_scan]-A[j]>a_value&&A[j-a_scan-1]-A[j-1]>a_value)
				{
					al_flag=3;
					al_edge=j-a_halfscan; 
					break;
...
//	for(i=b_start;i<br_end;i++)  //尝试两摄像头各扫一侧 r与l反
//		{
//			if(B[i]>b_T)
//				br_count++;
//			if(B[i]-B[i+b_scan]>b_value&&B[i+1]-B[i+b_scan+1]>b_value)
